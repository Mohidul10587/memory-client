...
export default function EditProfilePage() {
  ...
  const [formData, setFormData] = useState({
    name: '', sscPassingYear: '', joiningYear: '', subject: '',
    designation: '', currentAddress: '', permanentAddress: '',
    profession: '', workplace: '', email: '', facebookProfile: '',
    bloodGroup: '' as BloodGroup | '', about: '',
  });
  ...
  const { trigger, isMutating } = useSWRMutation(
    ...
    },
    {
      ...
      onError: (err: Error) => setError(err.message || 'আপডেট ব্যর্থ হয়েছে'),
    },
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isStudent) {
      ...
      delete payload.joiningYear; delete payload.subject; delete payload.designation;
    } else {
      payload.joiningYear = parseInt(formData.joiningYear);
      delete payload.sscPassingYear; delete payload.profession; delete payload.workplace;
    }
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
        ...
        <div>
          ...
          <textarea name="about" value={formData.about} onChange={handleChange} rows={4}
            className="input-field resize-none" placeholder="নিজের সম্পর্কে লিখুন..." />
        </div>
