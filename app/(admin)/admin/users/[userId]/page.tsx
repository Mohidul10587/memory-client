...
export default function AdminUserEditPage() {
  ...
  useEffect(() => {
    ...
    if (user.role === 'STUDENT' && user.studentProfile) {
      ...
      setFormData({
        name: p.name ?? '',
        sscPassingYear: p.sscPassingYear ?? '',
        division: p.division ?? '',
        district: p.district ?? '',
        upazila: p.upazila ?? '',
        union: p.union ?? '',
  ...
  const updateMutation = useMutation({
    mutationFn: () => {
      ...
      if (user?.role === 'STUDENT') {
        const sscYear = payload.sscPassingYear;
        return adminUsersApi.updateStudent(userId, {
          ...payload,
          sscPassingYear: sscYear ? Number(sscYear) : undefined,
        });
      } else {
        const joinYear = payload.joiningYear;
        return adminUsersApi.updateTeacher(userId, {
          ...payload,
          joiningYear: joinYear ? Number(joinYear) : undefined,
        });
      }
    },
    onSuccess: () => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-gray-900">
                {isStudent ? 'শিক্ষার্থী' : 'শিক্ষক'} সম্পাদনা
              </span>
            </div>
          </div>
          <button
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-5">
        ...
        {updateMutation.isError && (
          <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-700">
            {(updateMutation.error as any)?.message ?? 'কিছু একটা সমস্যা হয়েছে।'}
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center gap-4">
            {(user.studentProfile?.profileImage ?? user.teacherProfile?.profileImage) ? (
              <img
                src={user.studentProfile?.profileImage ?? user.teacherProfile?.profileImage}
                alt=""
            ...
            {/* Active toggle in card */}
            <div className="ml-auto flex items-center gap-2">
              <span className="text-xs text-gray-500">সক্রিয়:</span>
              <button
                type="button"
                onClick={() => setIsActive(!isActive)}
                className={`transition-colors ${isActive ? 'text-green-600' : 'text-red-400'}`}
              >
                {isActive ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {isStudent ? (
              <Field
                label="SSC পাসের বছর"
                type="number"
                value={String(formData.sscPassingYear ?? '')}
                onChange={(v) => set('sscPassingYear', v)}
              />
            ) : (
              <>
